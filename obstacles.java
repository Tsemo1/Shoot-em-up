...
public class obstacles {
	private float xObs,yObs,vitesseObs;//coordonn�es du centre et vitesse
	...
	public obstacles(float x, float y, float vitesse) throws SlickException {
		...
		imgObs = new Image("res/Asteroides.png");
	}
...
//lancer les obstacles sur le vaisseau
	...
	public boolean testCollision(Vaisseau v) {
		if(this.xObs >= v.getX()-v.getTaillex()/2 && this.xObs <= v.getX()+v.getTaillex()/2 && this.yObs >= v.getY()-v.getTailley()/2 && this.yObs <= v.getY()+v.getTailley()/2)//les coordonnees du projectile doivent etre comprises dans les dimensions du vaisseau
			return true;
		else
			return false;
	}
